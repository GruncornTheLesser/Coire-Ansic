 ...
 * could distinguish between strict resource and 
...
resource::pool<T>& resource::dynamic_registry::get_resource_pool(Args&& ... args)
{
	...
	if (it == data.end())
	{
		if constexpr (std::is_constructible_v<T, Args...>)
			...
			throw std::runtime_error("registry does not contain resource '" + util::type_name<T>().data() + "' and cannot construct with from arguments: "
